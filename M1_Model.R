...
dataset_file <- "Maize"
predictors <- c("Line")
# predictors <- c("Line", "LinexEnv")
predictors_text <- paste0(predictors, collapse = " + ")
iterations_number <- 10000
burn_in <- 5000
base_results_dir <- "results"
type <- if (length(predictors) == 2) "M1_GE" else "M1_NO_GE"
cores_num <- 3
...
for (i in seq_along(data_info$traits)) {
  ...
  AllPredictions <- foreach(
    ...
    Env <- dummy_matrix(Pheno$Env, remove_level = fold$testing_env)
    Line <- dummy_matrix(Pheno$Line)
    GenoLine <- Line %*% Geno %*% t(Line)
    KE <- Env %*% t(Env)
    LinexEnv <- GenoLine * KE
    ...
    data.frame(
      ...
      Predicted = model$yHat[fold$testing]
    )
  }
